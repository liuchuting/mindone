...
class ModifiedResNet(nn.Cell):
    ...
    def __init__(self, layers, output_dim, heads, input_resolution=224, width=64, dtype=ms.float32):
        ...
        self.relu2 = mint.nn.ReLU()
        self.conv3 = mint.nn.Conv2d(width // 2, width, kernel_size=3, padding=1, bias=False).to_float(
            self.dtype
        )
        ...
        self.mlp = nn.SequentialCell(
            OrderedDict(
                [
                    ...
                    ("gelu", QuickGELU().to_float(self.dtype) if use_quick_gelu else mint.nn.GELU().to_float(self.dtype)),
