...
def shift_tokens_right(
    input_ids: ms.Tensor, pad_token_id: int, decoder_start_token_id: int
):
    """
...
def create_position_ids_from_input_ids(
    ...
    incremental_indices = (
        mint.cumsum(mask, dim=1).type_as(mask) + past_key_values_length
    ) * mask
    return incremental_indices.long() + padding_idx


class M2M100ScaledWordEmbedding(mint.nn.Embedding):
    """
...
class M2M100SinusoidalPositionalEmbedding(nn.Cell):
    ...
    def get_embedding(
        ...
        emb = mint.arange(num_embeddings, dtype=ms.int64).float().unsqueeze(
            1
        ) * emb.unsqueeze(0)
...
class M2M100Attention(nn.Cell):
    ...
    def construct(
        ...
        attn_probs = mint.nn.functional.dropout(
...
class M2M100EncoderLayer(nn.Cell):
    ...
    def construct(
        ...
        hidden_states = mint.nn.functional.dropout(
...
class M2M100Encoder(M2M100PreTrainedModel):
    ...
    def __init__(
        self, config: M2M100Config, embed_tokens: Optional[mint.nn.Embedding] = None
    ):
