...
def shift_tokens_right(input_ids: ms.Tensor, pad_token_id: int,
...
def create_position_ids_from_input_ids(input_ids, padding_idx,
    ...
    incremental_indices = (mint.cumsum(mask, dim=1).type_as(mask) +
...
class M2M100ScaledWordEmbedding(nn.Embedding):
    ...
    def __init__(self, num_embeddings: int, embedding_dim: int, padding_idx:
        int, embed_scale: Optional[float]=1.0):
...
class M2M100SinusoidalPositionalEmbedding(nn.Module):
    ...
    def __init__(self, num_positions: int, embedding_dim: int, padding_idx:
        Optional[int]=None):
        super().__init__()
        self.offset = 2
    ...
    def make_weights(self, num_embeddings: int, embedding_dim: int,
        ...
        if hasattr(self, 'weights'):
            emb_weights = emb_weights.to(dtype=self.weights.dtype, device=
                self.weights.device)
    ...
    def get_embedding(num_embeddings: int, embedding_dim: int, padding_idx:
        ...
        emb = mint.arange(num_embeddings, dtype=ms.int64).float().unsqueeze(1
            ) * emb.unsqueeze(0)
        emb = mint.cat([mint.sin(emb), mint.cos(emb)], dim=1).view(
            num_embeddings, -1)
    ...
    @ms._no_grad()
    def construct(self, input_ids: ms.Tensor=None, inputs_embeds: ms.Tensor
        =None, past_key_values_length: int=0):
        if input_ids is not None:
            bsz, seq_len = input_ids.size()
            position_ids = create_position_ids_from_input_ids(input_ids,
                self.padding_idx, past_key_values_length)
        else:
    ...
    def create_position_ids_from_inputs_embeds(self, inputs_embeds,
        ...
        position_ids = mint.arange(self.padding_idx + 1, sequence_length +
            self.padding_idx + 1, dtype=ms.int64, device=inputs_embeds.device)
...
class M2M100Attention(nn.Module):
    ...
    def __init__(self, embed_dim: int, num_heads: int, dropout: float=0.0,
        is_decoder: bool=False, bias: bool=True, is_causal: bool=False,
    ...
    def construct(self, hidden_states: ms.Tensor, key_value_states:
        Optional[ms.Tensor]=None, past_key_value: Optional[Tuple[ms.Tensor]
        ]=None, attention_mask: Optional[ms.Tensor]=None, layer_head_mask:
        Optional[ms.Tensor]=None, output_attentions: bool=False) ->Tuple[
        ms.Tensor, Optional[ms.Tensor], Optional[Tuple[ms.Tensor]]]:
        ...
        attn_output = attn_output.view(bsz, self.num_heads, tgt_len, self.
            head_dim)
...
class M2M100FlashAttention2(M2M100Attention):
    ...
    def construct(self, hidden_states: ms.Tensor, key_value_states:
        ...
        if (is_cross_attention and past_key_value is not None and 
            ...
            key_states = mint.cat([past_key_value[0].transpose(1, 2),
                key_states], dim=1)
            value_states = mint.cat([past_key_value[1].transpose(1, 2),
                value_states], dim=1)
        else:
            key_states = self._reshape(self.k_proj(hidden_states), -1, bsz)
            value_states = self._reshape(self.v_proj(hidden_states), -1, bsz)
        ...
        attn_output = _flash_attention_forward(query_states, key_states,
            value_states, attention_mask, q_len, dropout=self.dropout if
            self.training else 0.0, is_causal=self.is_causal,
...
class M2M100SdpaAttention(M2M100Attention):
    ...
    def construct(self, hidden_states: ms.Tensor, key_value_states:
        ...
        is_causal = (True if self.is_causal and attention_mask is None and 
            tgt_len > 1 else False)
        attn_output = torch.nn.functional.scaled_dot_product_attention(
            query_states, key_states, value_states, attn_mask=
            attention_mask, dropout_p=self.dropout if self.training else 
            0.0, is_causal=is_causal)
        if attn_output.size() != (bsz, self.num_heads, tgt_len, self.head_dim):
            raise ValueError(
                f'`attn_output` should be of size {bsz, self.num_heads, tgt_len, self.head_dim}, but is {attn_output.size()}'
                )
...
class M2M100EncoderLayer(nn.Module):
    ...
    def __init__(self, config: M2M100Config):
        ...
        self.self_attn = M2M100_ATTENTION_CLASSES[config._attn_implementation](
            embed_dim=self.embed_dim, num_heads=config.
    ...
    def construct(self, hidden_states: ms.Tensor, attention_mask: ms.Tensor,
        layer_head_mask: ms.Tensor, output_attentions: bool=False) ->ms.Tensor:
        """
        Args:
        ...
        hidden_states, attn_weights, _ = self.self_attn(hidden_states=
            hidden_states, attention_mask=attention_mask, layer_head_mask=
            layer_head_mask, output_attentions=output_attentions)
        ...
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout,
            training=self.training)
        hidden_states = residual + hidden_states
        if hidden_states.dtype == ms.float16 and (mint.isinf(hidden_states)
            .any() or mint.isnan(hidden_states).any()):
            clamp_value = torch.finfo(hidden_states.dtype).max - 1000
            hidden_states = mint.clamp(hidden_states, min=-clamp_value, max
                =clamp_value)
...
M2M100_ATTENTION_CLASSES = {'eager': M2M100Attention, 'flash_attention_2':
    M2M100FlashAttention2, 'sdpa': M2M100SdpaAttention}
...
class M2M100DecoderLayer(nn.Module):
    ...
    def __init__(self, config: M2M100Config):
        ...
        self.activation_dropout = config.activation_dropout
        self.self_attn_layer_norm = nn.LayerNorm(self.embed_dim)
        self.encoder_attn = M2M100_ATTENTION_CLASSES[config.
            _attn_implementation](self.embed_dim, config.
            decoder_attention_heads, dropout=config.attention_dropout,
    ...
    def construct(self, hidden_states: ms.Tensor, attention_mask: Optional[
        ms.Tensor]=None, encoder_hidden_states: Optional[ms.Tensor]=None,
        encoder_attention_mask: Optional[ms.Tensor]=None, layer_head_mask:
        Optional[ms.Tensor]=None, cross_attn_layer_head_mask: Optional[
        ms.Tensor]=None, past_key_value: Optional[Tuple[ms.Tensor]]=None,
        ...
        hidden_states, self_attn_weights, present_key_value = self.self_attn(
            hidden_states=hidden_states, past_key_value=
            self_attn_past_key_value, attention_mask=attention_mask,
            layer_head_mask=layer_head_mask, output_attentions=
            output_attentions)
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout,
            training=self.training)
        hidden_states = residual + hidden_states
        ...
        if encoder_hidden_states is not None:
            ...
            (hidden_states, cross_attn_weights, cross_attn_present_key_value
                ) = (self.encoder_attn(hidden_states=hidden_states,
                key_value_states=encoder_hidden_states, attention_mask=
                encoder_attention_mask, layer_head_mask=
...
class M2M100Encoder(M2M100PreTrainedModel):
    ...
    def __init__(self, config: M2M100Config, embed_tokens: Optional[nn.
        Embedding]=None):
        ...
        self.embed_tokens = M2M100ScaledWordEmbedding(config.vocab_size,
            embed_dim, self.padding_idx, embed_scale=embed_scale)
        if embed_tokens is not None:
            self.embed_tokens.weight = embed_tokens.weight
        self.embed_positions = M2M100SinusoidalPositionalEmbedding(config.
            max_position_embeddings, embed_dim, self.padding_idx)
        self.layers = nn.ModuleList([M2M100EncoderLayer(config) for _ in
            range(config.encoder_layers)])
        self.layer_norm = nn.LayerNorm(config.d_model)
        self._use_flash_attention_2 = (config._attn_implementation ==
            'flash_attention_2')
        self._use_sdpa = config._attn_implementation == 'sdpa'
        self.gradient_checkpointing = False
        self.post_init()

    def construct(self, input_ids: Optional[ms.Tensor]=None, attention_mask:
        Optional[ms.Tensor]=None, head_mask: Optional[ms.Tensor]=None,
        inputs_embeds: Optional[ms.Tensor]=None, output_attentions:
        ...
        if attention_mask is not None:
            if self._use_flash_attention_2:
                attention_mask = (attention_mask if 0 in attention_mask else
                    None)
        ...
        synced_gpus = is_deepspeed_zero3_enabled() or is_fsdp_managed_module(
            self)
        for idx, encoder_layer in enumerate(self.layers):
            ...
            skip_the_layer = (True if self.training and dropout_probability <
                self.layerdrop else False)
            if not skip_the_layer or synced_gpus:
                if self.gradient_checkpointing and self.training:
                    layer_outputs = self._gradient_checkpointing_func(
                        encoder_layer.__call__, hidden_states,
                        attention_mask, head_mask[idx] if head_mask is not
                        None else None, output_attentions)
                else:
                    layer_outputs = encoder_layer(hidden_states,
                        attention_mask, layer_head_mask=head_mask[idx] if 
                        head_mask is not None else None, output_attentions=
                        output_attentions)
...
class M2M100Decoder(M2M100PreTrainedModel):
    ...
    def __init__(self, config: M2M100Config, embed_tokens: Optional[nn.
        ...
        self.embed_tokens = M2M100ScaledWordEmbedding(config.vocab_size,
            config.d_model, self.padding_idx, embed_scale=embed_scale)
        if embed_tokens is not None:
            self.embed_tokens.weight = embed_tokens.weight
        self.embed_positions = M2M100SinusoidalPositionalEmbedding(config.
            max_position_embeddings, config.d_model, self.padding_idx)
        self.layers = nn.ModuleList([M2M100DecoderLayer(config) for _ in
    ...
    def construct(self, input_ids: Optional[ms.Tensor]=None, attention_mask:
        Optional[ms.Tensor]=None, encoder_hidden_states: Optional[ms.Tensor
        ]=None, encoder_attention_mask: Optional[ms.Tensor]=None, head_mask:
        Optional[ms.Tensor]=None, cross_attn_head_mask: Optional[ms.Tensor]
        =None, past_key_values: Optional[List[ms.Tensor]]=None,
        inputs_embeds: Optional[ms.Tensor]=None, use_cache: Optional[bool]=
        ...
        if inputs_embeds is None:
            inputs_embeds = self.embed_tokens(input_ids)
        if self._use_flash_attention_2:
            combined_attention_mask = (attention_mask if attention_mask is not
                None and 0 in attention_mask else None)
            ...
            combined_attention_mask = _prepare_4d_causal_attention_mask(
                ...
                past_key_values_length)
        if (encoder_hidden_states is not None and encoder_attention_mask is not
            None):
            if self._use_flash_attention_2:
                encoder_attention_mask = (encoder_attention_mask if 0 in
                    encoder_attention_mask else None)
            elif self._use_sdpa and cross_attn_head_mask is None and not output_attentions:
                encoder_attention_mask = _prepare_4d_attention_mask_for_sdpa(
                    encoder_attention_mask, inputs_embeds.dtype, tgt_len=
                    input_shape[-1])
            else:
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if not skip_the_layer or synced_gpus:
                ...
                if self.gradient_checkpointing and self.training:
                    layer_outputs = self._gradient_checkpointing_func(
                        ...
                        combined_attention_mask, encoder_hidden_states,
                        encoder_attention_mask, head_mask[idx] if head_mask
                         is not None else None, cross_attn_head_mask[idx] if
                        cross_attn_head_mask is not None else None, None,
                    ...
                    layer_outputs = decoder_layer(hidden_states,
                        ...
                        layer_head_mask=head_mask[idx] if head_mask is not
                        None else None, cross_attn_layer_head_mask=
                        cross_attn_head_mask[idx] if cross_attn_head_mask
                         is not None else None, past_key_value=
            ...
            if skip_the_layer:
                continue
            if use_cache:
                next_decoder_cache += layer_outputs[3 if output_attentions else
                    1],
            if output_attentions:
        ...
        if not return_dict:
            return tuple(v for v in [hidden_states, next_cache,
                all_hidden_states, all_self_attns, all_cross_attentions] if
                v is not None)
        return BaseModelOutputWithPastAndCrossAttentions(last_hidden_state=
            hidden_states, past_key_values=next_cache, hidden_states=
            all_hidden_states, attentions=all_self_attns, cross_attentions=
...
class M2M100Model(M2M100PreTrainedModel):
    ...
    def __init__(self, config: M2M100Config):
        ...
        self.shared = M2M100ScaledWordEmbedding(vocab_size, config.d_model,
            padding_idx, embed_scale=embed_scale)
    ...
    @add_code_sample_docstrings(checkpoint=_CHECKPOINT_FOR_DOC, output_type
        =Seq2SeqModelOutput, config_class=_CONFIG_FOR_DOC)
    def construct(self, input_ids: Optional[ms.Tensor]=None, attention_mask:
        Optional[ms.Tensor]=None, decoder_input_ids: Optional[ms.Tensor]=
        None, decoder_attention_mask: Optional[ms.Tensor]=None, head_mask:
        Optional[ms.Tensor]=None, decoder_head_mask: Optional[ms.Tensor]=
        None, cross_attn_head_mask: Optional[ms.Tensor]=None,
        encoder_outputs: Optional[Tuple[Tuple[ms.Tensor]]]=None,
        past_key_values: Optional[Tuple[Tuple[ms.Tensor]]]=None,
        inputs_embeds: Optional[ms.Tensor]=None, decoder_inputs_embeds:
        Optional[ms.Tensor]=None, use_cache: Optional[bool]=None,
        output_attentions: Optional[bool]=None, output_hidden_states:
        Optional[bool]=None, return_dict: Optional[bool]=None) ->Union[
        Tuple[ms.Tensor], Seq2SeqModelOutput]:
        output_attentions = (output_attentions if output_attentions is not
            None else self.config.output_attentions)
        output_hidden_states = (output_hidden_states if 
            output_hidden_states is not None else self.config.
        ...
        if encoder_outputs is None:
            encoder_outputs = self.encoder(input_ids=input_ids,
                ...
                inputs_embeds=inputs_embeds, output_attentions=
                output_attentions, output_hidden_states=
                output_hidden_states, return_dict=return_dict)
        elif return_dict and not isinstance(encoder_outputs, BaseModelOutput):
            encoder_outputs = BaseModelOutput(last_hidden_state=
                encoder_outputs[0], hidden_states=encoder_outputs[1] if len
                (encoder_outputs) > 1 else None, attentions=encoder_outputs
                [2] if len(encoder_outputs) > 2 else None)
        decoder_outputs = self.decoder(input_ids=decoder_input_ids,
            attention_mask=decoder_attention_mask, encoder_hidden_states=
            encoder_outputs[0], encoder_attention_mask=attention_mask,
            head_mask=decoder_head_mask, cross_attn_head_mask=
            cross_attn_head_mask, past_key_values=past_key_values,
            inputs_embeds=decoder_inputs_embeds, use_cache=use_cache,
            output_attentions=output_attentions, output_hidden_states=
            output_hidden_states, return_dict=return_dict)
        if not return_dict:
            return decoder_outputs + encoder_outputs
        return Seq2SeqModelOutput(last_hidden_state=decoder_outputs.
            last_hidden_state, past_key_values=decoder_outputs.
            past_key_values, decoder_hidden_states=decoder_outputs.
            hidden_states, decoder_attentions=decoder_outputs.attentions,
...
class M2M100ForConditionalGeneration(M2M100PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: M2M100Config):
        ...
        self.lm_head = nn.Linear(config.d_model, self.model.shared.
            num_embeddings, bias=False)
        self.post_init()
    ...
    def construct(self, input_ids: Optional[ms.Tensor]=None, attention_mask:
        ...
        outputs = self.model(input_ids, attention_mask=attention_mask,
            ...
            decoder_inputs_embeds=decoder_inputs_embeds, use_cache=
...
__all__ = ['M2M100ForConditionalGeneration', 'M2M100Model',
